...
use group::{CsRng, GroupElement, PartyID, StatisticalSecuritySizedNumber};
...
use crate::ecdsa::sign::centralized_party::signature_homomorphic_evaluation_round::generate_protocol_contexts;
...
use crate::{dkg, ecdsa::presign, ecdsa::sign, languages, Error};
...
impl Party {
    ...
    /// This function implements steps  (2d) of the Sign protocol:
    ...
    ///
    /// Note: `hashed_message` is a `Scalar` which must be a
    ...
    // $\pi_{R_{B}}$
        ...
        GroupElement,
        GroupElement,
    )> where         Uint<SCALAR_LIMBS>: Encoding + ConcatMixed<StatisticalSecuritySizedNumber> + for<'a> From<&'a <Uint<SCALAR_LIMBS> as ConcatMixed<StatisticalSecuritySizedNumber>>::MixedOutput> + for<'a> From<&'a <Uint<SCALAR_LIMBS> as ConcatMixed<StatisticalSecuritySizedNumber>>::MixedOutput>,
    {
        // $ R_B$
