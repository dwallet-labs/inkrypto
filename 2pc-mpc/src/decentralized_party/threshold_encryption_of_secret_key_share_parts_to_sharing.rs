...
pub struct NonAggregatedPublicOutput {
    ...
    /// These are commitments to the coefficients of s(x) = x + s_1*x + ... + s_{t-1}*x^{t-1}
    /// where s(i) = [x]_i is party i's secret key share.
    ...
    // ==================== PVSS Dealings (encrypted randomizer shares) ====================
    /// PVSS dealings for secp256k1 randomizer shares (dealer party ID -> dealings).
    pub secp256k1_randomizer_dealings: HashMap<PartyID, Secp256k1CurveDealing>,
    /// PVSS dealings for ristretto randomizer shares (dealer party ID -> dealings).
    pub ristretto_randomizer_dealings: HashMap<PartyID, RistrettoCurveDealing>,
    /// PVSS dealings for curve25519 randomizer shares (dealer party ID -> dealings).
    pub curve25519_randomizer_dealings: HashMap<PartyID, Curve25519CurveDealing>,
    /// PVSS dealings for secp256r1 randomizer shares (dealer party ID -> dealings).
    pub secp256r1_randomizer_dealings: HashMap<PartyID, Secp256r1CurveDealing>,
...
// ==================== Helper functions for deriving Shamir shares ====================

/// Generic function to compute Shamir shares of secret key share parts for any curve.
///
/// Delegates to the subprotocol's `derive_shamir_share_of_secret` for each secret,
...
where
    ...
    >,
{
    let first_dealings: Vec<_> = dealings
        .values()
        .map(|d| &d.first_randomizer_contribution_dealing)
        .collect();

    let first_share = class_groups::threshold_encryption_to_sharing::derive_shamir_share_of_secret::<
        SCALAR_LIMBS,
        FUNDAMENTAL_DISCRIMINANT_LIMBS,
        NON_FUNDAMENTAL_DISCRIMINANT_LIMBS,
        GroupElement,
    ...
    let second_dealings: Vec<_> = dealings
        .values()
        .map(|d| &d.second_randomizer_contribution_dealing)
        .collect();

    let second_share =
        class_groups::threshold_encryption_to_sharing::derive_shamir_share_of_secret::<
            SCALAR_LIMBS,
            FUNDAMENTAL_DISCRIMINANT_LIMBS,
            NON_FUNDAMENTAL_DISCRIMINANT_LIMBS,
            GroupElement,
