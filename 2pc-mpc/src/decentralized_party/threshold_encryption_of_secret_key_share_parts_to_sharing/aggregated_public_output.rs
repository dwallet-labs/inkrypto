...
//!
//! The protocol's rounds exchange every honest dealer's full PVSS dealing — per-receiver
...
//! sharings add coefficient-wise, output formation homomorphically sums each receiver's
//! encrypted shares across the honest dealers into one ciphertext per (curve, key-share
...
///
/// A party with no aggregated entry received no dealt shares, which corresponds to a neutral
/// randomizer (there was nothing to sum), so the share is the masked secret itself.
///
...
mod tests {
    ...
    /// A structurally-valid all-neutral/empty aggregated sharing output.
    fn neutral_public_output() -> PublicOutput {
    ...
    #[test]
    fn round_trips_bcs_bytes() {
        let public_output = neutral_public_output();

        let bytes = bcs::to_bytes(&public_output).unwrap();
        let deserialized: PublicOutput = bcs::from_bytes(&bytes).unwrap();
        assert_eq!(public_output, deserialized);
    }
}
