    ...
    Error: From<DecryptionKeyShare::Error>,
{
    ...
    pub(super) fn evaluate_encryption_of_signature_response(
        ...
        let challenge =
            public_key.derive_challenge(&public_nonce, message, hash_scheme, hash_context)?;
        ...
        let encryption_of_signature_response = encryption_of_nonce_share
            .add_vartime(
                &encryption_of_secret_key_share
                    .scale_vartime(&challenge, ciphertext_space_public_parameters),
                ciphertext_space_public_parameters,
            )
            .add_vartime(
                &encryption_of_centralized_party_partial_response,
                ciphertext_space_public_parameters,
            );
