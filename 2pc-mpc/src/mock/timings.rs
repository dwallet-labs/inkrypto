...
fn mock_protocol_timings_report() {
    ...
    // The dWallet DKG and presign mocks (like the sign parties above) finalize in round one; the
    // party `advance` wrappers add nothing beyond the timed output construction.
    ...
    report(
        "ECDSA presign (secp256k1): full party advance (finalize round one)",
        200,
        || {
