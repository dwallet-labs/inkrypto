...
///
/// Backward-compatible copy of the pre-Protocol-0.1 `decentralized_party::reconfiguration::PublicInput`
/// shape, so the v0.1 protocol path can be driven without supplying the newer PVSS / threshold-encryption-to-sharing fields.
...
pub struct Party {}
/// INSECURE `unsafe_mock`: the real multi-round, multi-CRT-prime backward-compatible (V2) network
/// reconfiguration party is replaced by an instant mock that finalizes with the `core` of the
/// canonical `NETWORK_KEY_SEED` reconfiguration output.
#[cfg(feature = "unsafe_mock")]
pub use crate::mock::network_dkg::MockNetworkReconfigurationPartyV2 as Party;
...
impl AsynchronouslyAdvanceable for Party {
    ...
    > {
        ...
        let discrete_log_group_public_parameters =
            bounded_integers_group::PublicParameters::new_with_randomizer_upper_bound_backward_compatible(
                randomizer_share_bits,
            )?;
