...
use proof_aggregation::synchronous::CommitmentRoundParty;

use crate::decommitment_round;
use crate::decommitment_round::Decommitment;
    ...
    for Party<REPETITIONS, Language, ProtocolContext>
{
    ...
    fn commit_statements_and_statement_mask(
        ...
        if !self.provers.contains(&self.party_id) {
            return Err(Error::from(ErrorKind::Aggregation(
                proof_aggregation::synchronous::Error::from(
                    proof_aggregation::synchronous::ErrorKind::NonParticipatingParty,
                ),
            )));
        }
    ...
    > Party<REPETITIONS, Language, ProtocolContext>
{
    pub fn new_session(
        ...
        let (randomizers, statement_masks) = maurer::Proof::<
            REPETITIONS,
            Language,
            ProtocolContext,
