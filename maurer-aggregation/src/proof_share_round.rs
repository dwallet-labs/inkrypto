...
use crate::{
    commitment_round::COMMITMENT_LABEL, decommitment_round::Decommitment, proof_aggregation_round,
    Error, ErrorKind, Result,
};
use commitment::Commitment;
use group::helpers::DeduplicateAndSort;
use group::{helpers::FlatMapResults, CsRng, GroupElement, PartyID};
use maurer::language::WitnessSpaceValue;
use mpc::HandleInvalidMessages;
use proof::GroupsPublicParametersAccessors;
use proof_aggregation::synchronous::{process_incoming_messages, ProofShareRoundParty};
...
pub struct Party<
    ...
    Language: maurer::Language<REPETITIONS>,
    ...
    fn generate_proof_share(
        ...
        let reconstructed_commitments: Result<HashMap<PartyID, Commitment>> = decommitments
            ...
            .map(|(party_id, decommitment)| {
                maurer::Proof::<REPETITIONS, Language, ProtocolContext>::setup_transcript(
        ...
        let miscommitting_parties: Vec<PartyID> = decommitments
            .keys()
            .cloned()
        ...
        if !miscommitting_parties.is_empty() {
            return Err(proof_aggregation::synchronous::Error::from(
                proof_aggregation::synchronous::ErrorKind::WrongDecommitment(miscommitting_parties),
            ))?;
        }
        ...
        let parties_sending_invalid_statement_masks: Vec<PartyID> = statement_masks
            .iter()
        ...
        if !parties_sending_invalid_statement_masks.is_empty() {
            return Err(proof_aggregation::synchronous::Error::from(
                proof_aggregation::synchronous::ErrorKind::InvalidDecommitment(
        ...
        if !parties_committed_on_wrong_number_of_statements.is_empty() {
            return Err(proof_aggregation::synchronous::Error::from(
                proof_aggregation::synchronous::ErrorKind::WrongNumberOfDecommittedStatements(
            ...
            |aggregated_statement_masks, statement_masks| {
                aggregated_statement_masks
                    ...
                    .map(|(aggregated_statement_mask, statement_mask)| {
                        aggregated_statement_mask.add_vartime(
                            &statement_mask,
                            self.language_public_parameters
                                .statement_space_public_parameters(),
                        )
        ...
        let statements: HashMap<PartyID, Vec<_>> = decommitments
            .clone()
            .into_iter()
            .map(|(party_id, decommitment)| {
                (
        ...
        let responses = maurer::Proof::<REPETITIONS, Language, ProtocolContext>::prove_inner(
            &self.protocol_context,
            &self.language_public_parameters,
            self.witnesses,
        ...
        let proof_share = ProofShare(responses.clone());
...
impl<const REPETITIONS: usize, Language: maurer::Language<REPETITIONS>>
