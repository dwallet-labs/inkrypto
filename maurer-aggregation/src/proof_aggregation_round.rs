...
use crate::proof_share_round::ProofShare;
use crate::Output;
...
use proof_aggregation::synchronous::{process_incoming_messages, ProofAggregationRoundParty};
    ...
    fn aggregate_proof_shares(
                    ...
                    |aggregated_responses, proof_share| {
                        aggregated_responses
                            ...
                            .map(|(aggregated_response, response)| {
                                aggregated_response.add_vartime(
                                    response,
                                    self.language_public_parameters
                                        .witness_space_public_parameters(),
                                )
        ...
        let aggregated_statement_masks =
            Language::StatementSpaceGroupElement::batch_normalize_const_generic(
                self.aggregated_statement_masks.clone(),
            );
        let aggregated_proof =
            maurer::Proof::new(aggregated_statement_masks.clone(), aggregated_responses);
        if aggregated_proof
            .verify(
            ...
            .is_err()
        {
            ...
            */
            ...
            let challenges: [Vec<ComputationalSecuritySizedNumber>; REPETITIONS] =
                maurer::Proof::<REPETITIONS, Language, ProtocolContext>::compute_challenges(
            ...
            let proofs: HashMap<_, _> = proof_shares
                ...
                .map(|(party_id, proof_share)| {
                    (
                        party_id,
                        maurer::Proof::<REPETITIONS, Language, ProtocolContext>::new(
                            // Same parties participating in all rounds, safe to `.unwrap()`.
                            self.statement_masks.get(&party_id).unwrap().clone(),
                            Language::WitnessSpaceGroupElement::batch_normalize_const_generic(
                                proof_share,
                            ),
                        ),
