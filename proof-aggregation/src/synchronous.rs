...
/// Synchronous-aggregation error wrapper that carries a backtrace captured at construction.
...
/// Synchronous-aggregation error kind.
...
pub enum ErrorKind {
    ...
    #[error(
        "parties {:?} maliciously attempted to bypass the commitment round by sending decommitment which does not match their commitment", .0
    )]
...
impl From<Error> for mpc::Error {
    fn from(value: Error) -> Self {
        match value.kind {
            ErrorKind::NonParticipatingParty => {
                mpc::Error::from(mpc::ErrorKind::NonParticipatingParty)
            }
            ErrorKind::UnresponsiveParties(parties) => {
                mpc::Error::from(mpc::ErrorKind::UnresponsiveParties(parties))
            }
            ErrorKind::InvalidCommitment(parties) => {
                mpc::Error::from(mpc::ErrorKind::InvalidMessage(parties))
            }
            ErrorKind::InvalidDecommitment(parties) => {
                mpc::Error::from(mpc::ErrorKind::InvalidMessage(parties))
            }
            ErrorKind::WrongDecommitment(parties) => {
                mpc::Error::from(mpc::ErrorKind::MaliciousMessage(parties))
            }
            ErrorKind::WrongNumberOfDecommittedStatements(parties) => {
                mpc::Error::from(mpc::ErrorKind::InvalidMessage(parties))
            }
            ErrorKind::InvalidProofShare(parties) => {
                mpc::Error::from(mpc::ErrorKind::InvalidMessage(parties))
            }
            ErrorKind::ProofShareVerification(parties) => {
                mpc::Error::from(mpc::ErrorKind::MaliciousMessage(parties))
            }
            ErrorKind::MPC(e) => e,
        }
...
pub fn process_incoming_messages<T>(
    ...
    let unresponsive_parties: Vec<PartyID> = other_provers
        .difference(&current_round_party_ids)
        .cloned()
        .deduplicate_and_sort();

    if !unresponsive_parties.is_empty() {
        return Err(Error::from(ErrorKind::UnresponsiveParties(
            ...
            .all(|(party_id, party)| {
                if miscommitting_parties.contains(&party_id) {
                    ...
                    matches!(
                        ...
                        Error { kind: ErrorKind::WrongDecommitment(parties), .. } if parties == miscommitting_parties
                    )
                }
            }));
    }
        ...
        assert!(proof_aggregation_round_parties
            ...
            .all(|(party_id, party)| {
                if misproving_parties.contains(&party_id) {
                    ...
                    matches!(
                        res.err().unwrap().try_into().unwrap(),
                        Error { kind: ErrorKind::ProofShareVerification(parties), .. } if parties == misproving_parties
                    )
                }
            }));
    }
        ...
        assert!(decommitment_round_parties
            ...
            .all(|(party_id, party)| {
                if unresponsive_parties.contains(&party_id) {
                    ...
                    matches!(
                        res.err().unwrap().try_into().unwrap(),
                        Error { kind: ErrorKind::UnresponsiveParties(parties), .. } if parties == unresponsive_parties
                    )
                }
            }));
