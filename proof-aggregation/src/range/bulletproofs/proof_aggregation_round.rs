...
use crate::{
    ...
    synchronous::{process_incoming_messages, ProofAggregationRoundParty},
    ...
    for Party<NUM_RANGE_CLAIMS>
{
    ...
    fn aggregate_proof_shares(
        ...
        let parties_sending_wrong_number_of_proof_shares: Vec<PartyID> = proof_shares
            .iter()
        ...
        if !parties_sending_wrong_number_of_proof_shares.is_empty() {
            return Err(crate::synchronous::Error::from(
                crate::synchronous::ErrorKind::InvalidProofShare(
            ...
            .map_err(|e| match e {
                MPCError::MalformedProofShares { bad_shares } => bad_shares
                    ...
                    .map(|malicious_parties| {
                        Error::from(ErrorKind::SynchronousAggregation(
                            crate::synchronous::Error::from(
                                crate::synchronous::ErrorKind::ProofShareVerification(
                                    malicious_parties,
                                ),
                            ),
                        ))
                    })
                    .unwrap_or(Error::from(ErrorKind::InternalError)),
                _ => Error::from(ErrorKind::InvalidParameters),
            })?;

        let range_proof = super::RangeProof::new_aggregated(
            proof,
            self.batch_size,
