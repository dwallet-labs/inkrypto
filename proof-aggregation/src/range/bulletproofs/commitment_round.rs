...
use crate::{range::CommitmentScheme, synchronous::CommitmentRoundParty, Error, ErrorKind, Result};
    ...
    for Party<NUM_RANGE_CLAIMS>
{
    ...
    fn commit_statements_and_statement_mask(
        ...
        if !self.provers.contains(&self.party_id) {
            return Err(Error::from(ErrorKind::SynchronousAggregation(
                crate::synchronous::Error::from(
                    crate::synchronous::ErrorKind::NonParticipatingParty,
                ),
            )));
        }
