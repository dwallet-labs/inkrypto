...
// Only the real per-curve keygen and the (test-only) fixture generator derive
// setup parameters; the `unsafe_mock` runtime path serves them from fixtures.
#[cfg(any(not(feature = "unsafe_mock"), test))]
...
/// Fixture generator (test-only, feature `unsafe_mock`): derives the curve's
/// setup parameters and returns the neutral encryption key + `new_default` proof
/// that the embedded PVSS fixtures record. This is the slow derivation the mock
/// keygen exists to avoid at runtime — it is compiled only for the
/// `regenerate_fixtures` test (see [`crate::publicly_verifiable_secret_sharing::
/// unsafe_mock_keygen_fixtures`]), never on any runtime path.
#[cfg(all(test, feature = "unsafe_mock"))]
#[allow(clippy::type_complexity)]
pub(crate) fn derive_mock_pvss_keypair<
    const PLAINTEXT_SPACE_SCALAR_LIMBS: usize,
    const FUNDAMENTAL_DISCRIMINANT_LIMBS: usize,
    const HALF_NON_FUNDAMENTAL_DISCRIMINANT_LIMBS: usize,
    const NON_FUNDAMENTAL_DISCRIMINANT_LIMBS: usize,
    ...
    PlaintextSpaceGroupElement: PrimeGroupElement<PLAINTEXT_SPACE_SCALAR_LIMBS>,
>(
    plaintext_space_public_parameters: group::PublicParameters<PlaintextSpaceGroupElement::Scalar>,
) -> Result<(
    CompactIbqf<NON_FUNDAMENTAL_DISCRIMINANT_LIMBS>,
    KnowledgeOfDecryptionKeyUCProof<
        DECRYPTION_KEY_WITNESS_LIMBS,
    ...
    >,
{
    let setup_parameters = SetupParameters::<
        ...
        NON_FUNDAMENTAL_DISCRIMINANT_LIMBS,
        group::PublicParameters<PlaintextSpaceGroupElement::Scalar>,
    >::derive_from_plaintext_parameters::<PlaintextSpaceGroupElement::Scalar>(
        plaintext_space_public_parameters,
        crate::DEFAULT_COMPUTATIONAL_SECURITY_PARAMETER,
    )?;
    ...
    let encryption_key = EquivalenceClass::neutral_from_public_parameters(
        language_public_parameters.statement_space_public_parameters(),
    )?
    .value();

    let proof = KnowledgeOfDecryptionKeyUCProof::<
        DECRYPTION_KEY_WITNESS_LIMBS,
        NON_FUNDAMENTAL_DISCRIMINANT_LIMBS,
    >::new_default(
        language_public_parameters.witness_space_public_parameters(),
    ...
    Ok((
        encryption_key,
        proof,
        Uint::from(crate::UNSAFE_MOCK_DECRYPTION_KEY),
    ))
}

/// Deterministically generate a per-curve PVSS HPKE keypair
/// from `rng`: the encryption key, its UC knowledge-of-decryption-key proof, and
/// the decryption key. Derives the curve's setup parameters internally from the
/// plaintext-space public parameters, so callers never touch the low-level setup
/// primitives.
#[cfg(not(feature = "unsafe_mock"))]
#[allow(clippy::type_complexity)]
pub fn generate_pvss_keypair<
    const PLAINTEXT_SPACE_SCALAR_LIMBS: usize,
    const FUNDAMENTAL_DISCRIMINANT_LIMBS: usize,
    const HALF_NON_FUNDAMENTAL_DISCRIMINANT_LIMBS: usize,
    const NON_FUNDAMENTAL_DISCRIMINANT_LIMBS: usize,
    ...
    >,
{
    ...
    generate_and_prove_encryption_keypair::<
        PLAINTEXT_SPACE_SCALAR_LIMBS,
        FUNDAMENTAL_DISCRIMINANT_LIMBS,
        HALF_NON_FUNDAMENTAL_DISCRIMINANT_LIMBS,
        NON_FUNDAMENTAL_DISCRIMINANT_LIMBS,
        DOUBLE_NON_FUNDAMENTAL_DISCRIMINANT_LIMBS,
        DECRYPTION_KEY_WITNESS_LIMBS,
        PlaintextSpaceGroupElement,
    >(&setup_parameters, rng)
}

/// A PVSS HPKE keypair for a single curve: the encryption key value,
/// its UC knowledge-of-decryption-key proof, and the (secret) decryption key.
pub type PvssKeypairAndProof<
    const FUNDAMENTAL_DISCRIMINANT_LIMBS: usize,
    const NON_FUNDAMENTAL_DISCRIMINANT_LIMBS: usize,
> = (
    CompactIbqf<NON_FUNDAMENTAL_DISCRIMINANT_LIMBS>,
...
/// Deterministically generate the three per-curve PVSS HPKE keypairs
/// — secp256k1, secp256r1, ristretto — from per-curve domain-separated RNGs. A
/// single entry point over the concrete curves so callers never touch setup
/// parameters or per-curve const generics.
#[cfg(not(feature = "unsafe_mock"))]
#[allow(clippy::type_complexity)]
pub fn generate_pvss_keypairs(
    secp256k1_rng: &mut impl CsRng,
    secp256r1_rng: &mut impl CsRng,
    ristretto_rng: &mut impl CsRng,
) -> Result<(
    PvssKeypairAndProof<
        { crate::SECP256K1_FUNDAMENTAL_DISCRIMINANT_LIMBS },
        { crate::SECP256K1_NON_FUNDAMENTAL_DISCRIMINANT_LIMBS },
    >,
    PvssKeypairAndProof<
        { crate::SECP256R1_FUNDAMENTAL_DISCRIMINANT_LIMBS },
        { crate::SECP256R1_NON_FUNDAMENTAL_DISCRIMINANT_LIMBS },
    >,
    PvssKeypairAndProof<
        { crate::RISTRETTO_FUNDAMENTAL_DISCRIMINANT_LIMBS },
        { crate::RISTRETTO_NON_FUNDAMENTAL_DISCRIMINANT_LIMBS },
    >,
)> {
    let secp256k1 = generate_pvss_keypair::<
    ...
    let secp256r1 = generate_pvss_keypair::<
    ...
    let ristretto = generate_pvss_keypair::<
    ...
    Ok((secp256k1, secp256r1, ristretto))
}

/// INSECURE mock twin (feature `unsafe_mock`): decodes the three per-curve neutral
/// encryption keys + default proofs from an embedded fixture (see
/// [`crate::publicly_verifiable_secret_sharing::unsafe_mock_keygen_fixtures`]) and
/// pairs each with a fixed decryption key — no runtime setup-parameter derivation.
/// The production definition above is left untouched.
#[cfg(feature = "unsafe_mock")]
#[allow(clippy::type_complexity)]
pub fn generate_pvss_keypairs(
    _secp256k1_rng: &mut impl CsRng,
    _secp256r1_rng: &mut impl CsRng,
    _ristretto_rng: &mut impl CsRng,
) -> Result<(
    PvssKeypairAndProof<
        { crate::SECP256K1_FUNDAMENTAL_DISCRIMINANT_LIMBS },
        { crate::SECP256K1_NON_FUNDAMENTAL_DISCRIMINANT_LIMBS },
    ...
    let ((secp256k1_key, secp256k1_proof), (secp256r1_key, secp256r1_proof), (ristretto_key, ristretto_proof)) =
        crate::publicly_verifiable_secret_sharing::unsafe_mock_keygen_fixtures::pvss_encryption_keys_and_proofs()?;
...
/// produces default proofs (see inkrypto `unsafe_mock`) that real
