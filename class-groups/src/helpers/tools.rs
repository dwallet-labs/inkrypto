...
use crypto_bigint::{ConstChoice, Limb, Odd, Uint};
...
pub(crate) trait Compact: Sized {
    ...
    fn section_vartime_length<const SECTION_LIMBS: usize>(
        &self,
        idx: u32,
        length: u32,
    ) -> Uint<SECTION_LIMBS>;
    ...
    fn section_vartime<const SECTION_LIMBS: usize>(
        &self,
        idx: u32,
        length: u32,
    ) -> Uint<SECTION_LIMBS>;
    ...
    fn compact<const K: u32, const SUMMARY_LIMBS: usize>(&self, n: u32) -> Uint<SUMMARY_LIMBS>;
}

impl<const LIMBS: usize> Compact for Uint<LIMBS> {
    #[inline(always)]
    fn section_vartime_length<const SECTION_LIMBS: usize>(
        &self,
        idx: u32,
...
pub(crate) trait BoundedDivMod2k: Sized {
    ...
    fn bounded_div_2k_mod_q(self, k: u32, k_bound: u32, q: &Odd<Self>) -> Self;

    /// Computes `self + (b * c) + carry`, returning the result along with the new carry.
    fn mac_limb(self, b: &Self, c: Limb, carry: Limb) -> (Self, Limb);
}

impl<const LIMBS: usize> BoundedDivMod2k for Uint<LIMBS> {
    /// Compute `self / 2^k mod q`.
    ///
    /// Executes in time variable in `k_bound`. This value should be chosen as an inclusive
    /// upperbound to the value of `k`.
    ...
    fn bounded_div_2k_mod_q(self, k: u32, k_bound: u32, q: &Odd<Self>) -> Self {
        ...
        while r < rounds {
            ...
            let (_, s) =
                x.as_limbs()[0].bounded_div2k_mod_q(f, f_bound, one_half_mod_q.as_limbs()[0]);
            ...
            x_qs_div_2f.as_mut_limbs()[LIMBS - 1] =
                x_qs_div_2f.as_limbs()[LIMBS - 1].bitxor(x_qs_hi);
    ...
    fn mac_limb(mut self, b: &Self, c: Limb, mut carry: Limb) -> (Self, Limb) {
        let mut i = 0;
        while i < LIMBS {
            (self.as_mut_limbs()[i], carry) =
                b.as_limbs()[i].carrying_mul_add(c, self.as_limbs()[i], carry);
            i += 1;
        }
        (self, carry)
    }
...
pub(crate) trait BoundedDiv2kModQ: Sized {
    /// Compute `self / 2^k mod q`. Returns the result, as well as a factor `f` such that `2^k`
    /// divides `self + q * f`.
    ///
    /// Executes in time variable in `k_bound`. This value should be chosen as an inclusive
    /// upperbound to the value of `k`.
    fn bounded_div2k_mod_q(self, k: u32, k_bound: u32, one_half_mod_q: Self) -> (Self, Self);
}

impl BoundedDiv2kModQ for Limb {
...
#[cfg(test)]
mod tests {
    use crypto_bigint::{Limb, Uint, U128};

    use crate::helpers::tools::{BoundedDiv2kModQ, BoundedDivMod2k};
    ...
    fn test_mac_limb() {
        ...
        assert_eq!(res, lo);
        assert_eq!(mac_carry, hi.as_limbs()[0]);
        assert_eq!(carry, Limb::ZERO)
    }
