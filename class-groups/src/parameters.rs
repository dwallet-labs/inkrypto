    ...
    Uint<DOUBLE_DELTA_QK_LIMBS>: Split<Output = Uint<DELTA_QK_LIMBS>>,
{
    pub(crate) fn new(
        ...
        let q_exp_2k = CtOption::from(q_exp_k.checked_mul(&q_exp_k))
            ...
            .into_option()
            .ok_or_else(|| Error::from(ErrorKind::InvalidDiscriminantParameters))?;
...
/// Table 2
pub(crate) fn minimum_discriminant_bits(computational_security_bits: u32) -> Result<u32, Error> {
...
mod tests {
    ...
    fn test_new_random_vartime_csp_too_high_errors() {
        ...
        matches!(
            res.unwrap_err().kind,
            ErrorKind::ComputationalSecurityTooHigh
        );
    }
